@unless ($isFollowed)
    <form action="{{route('wishlist.add', $product)}}" method="post" class="w-100">
        ...
        <div class="row w-100">
            @unless($mini)
                <label for="#" class="col-10 col-form-label">Notify when product will be available</label>
            @endunless
            <div class="col-2">
...
@else
    <form action="{{route('wishlist.remove', $product)}}" method="post" class="w-100">
        ...
        <div class="row w-100">
            @unless($mini)
                <label for="#" class="col-10 col-form-label">Product available unsubscribe</label>
            @endunless
            <div class="col-2">
